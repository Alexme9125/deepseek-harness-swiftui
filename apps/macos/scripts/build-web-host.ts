/**
 * Build the macOS bundled web-host executable. The deploy flags, symlink
 * materialization, and `@yao-pkg/pkg --sea` route match
 * scripts/build-exe-for-python-sdk.ts; this script does not reuse the
 * JSON-RPC / Python SDK deploy root.
 */
...
/** The closure manifest whose dependencies define the web-host executable. */
const DEPLOY_ROOT_PACKAGE = 'dsh-web-host-pkg'
...
/** Pinned for reproducible builds; same pin as the Python SDK exe. */
const PKG_SPEC = '@yao-pkg/pkg@6.21.0'
/** Product directory copied into the .app. */
const OUT_DIR = 'apps/macos/dist'
/** Deployed closure staging; also the node-mode carrier for local verification. */
const STAGING_DIR = 'apps/macos/dist/closure'
...
class Target {
  ...
  static parse(spec: string): Target {
    ...
    if (parts.length !== 3 || nodeRange === undefined || platform === undefined || arch === undefined) {
      throw new Error(`build-web-host: target ${JSON.stringify(spec)} must be <nodeRange>-<platform>-<arch>, e.g. node24-macos-arm64.`)
    }
    if (!/^node\d+$/.test(nodeRange)) {
      throw new Error(`build-web-host: target ${JSON.stringify(spec)}: node range must look like node24, got ${JSON.stringify(nodeRange)}.`)
    }
    if (!isPlatform(platform)) {
      throw new Error(`build-web-host: target ${JSON.stringify(spec)}: platform must be one of ${PLATFORMS.join(', ')}, got ${JSON.stringify(platform)}.`)
    }
    if (!isArch(arch)) {
      throw new Error(`build-web-host: target ${JSON.stringify(spec)}: arch must be one of ${ARCHES.join(', ')}, got ${JSON.stringify(arch)}.`)
    }
  ...
  static host(): Target {
    ...
    if (platform === undefined) {
      throw new Error(`build-web-host: unsupported host platform ${process.platform}; pass --targets explicitly.`)
    }
    const arch = process.arch === 'x64' || process.arch === 'arm64' ? process.arch : undefined
    if (arch === undefined) {
      throw new Error(`build-web-host: unsupported host arch ${process.arch}; pass --targets explicitly.`)
    }
...
class BuildCli {
  ...
  static parse(argv: string[]): BuildCli {
    ...
    try {
      ...
      console.error(`build-web-host: ${error instanceof Error ? error.message : String(error)}\n`)
    ...
    if (targets.length === 0) throw new Error('build-web-host: --targets is empty.')
    const seen = new Set<string>()
    for (const target of targets) {
      const key = `${target.platform}-${target.arch}`
      if (seen.has(key)) {
        throw new Error(`build-web-host: duplicate platform-arch ${key} in --targets; product names would collide.`)
      }
      seen.add(key)
    }
  ...
  private static usage(): string {
    return [
      'Usage: pnpm exec tsx apps/macos/scripts/build-web-host.ts [flags]',
      '',
      '  --targets=<t1,t2,...>  pkg targets, e.g. node24-macos-arm64.',
      '                         Default: the host platform only (on node24).',
      '  --skip-build           skip `pnpm run build` (lib/ artifacts must already exist).',
      '  --dry-run              print every command and config patch without executing.',
      '  --help                 print this help.',
      '',
      `Build route: ${PKG_SPEC} --sea. This is not the Python SDK JSON-RPC exe.`,
      `Writes the macOS product to ${OUT_DIR}/dsh-web-host.`,
    ].join('\n')
  }
}
...
class WebHostBuild {
  ...
  /** Verify the web-host closure before compiling or packaging. */
  ...
  async build(): Promise<void> {
    if (this.cli.skipBuild) {
      console.log('build-web-host: skipping pnpm run build (--skip-build)')
      return
    }
  ...
  async deployStaging(): Promise<void> {
    if (this.staging === root || root.startsWith(this.staging + sep)) {
      throw new Error(`build-web-host: refusing to clear staging dir ${this.staging}: it contains the repo root.`)
    }
    if (this.cli.dryRun) console.log(`build-web-host: [dry-run] rm -rf ${this.staging}`)
  ...
  private async restoreLegacyHoists(): Promise<void> {
    if (this.cli.dryRun) {
      console.log('build-web-host: [dry-run] restore direct dependencies omitted by legacy deploy')
      return
    }
    ...
    for (const dependency of Object.keys(manifest.dependencies ?? {}).sort()) {
      ...
      if (!existsSync(source)) {
        throw new Error(
          `build-web-host: deployed dependency ${dependency} is absent from both ${destination} and ${source}.`,
        )
      }
    ...
    if (stillMissing.length > 0) {
      throw new Error(`build-web-host: staged dependencies remain missing: ${stillMissing.join(', ')}.`)
    }
    if (restored.length > 0) {
      console.log(`build-web-host: restored legacy deploy hoists: ${restored.join(', ')}`)
    }
  }
  ...
  private async materializeStagedLinks(): Promise<void> {
    if (this.cli.dryRun) {
      console.log('build-web-host: [dry-run] materialize staged package links')
      return
    }
  ...
  async injectPkgConfig(): Promise<void> {
    ...
    const manifestPath = join(this.staging, 'package.json')
    if (this.cli.dryRun) {
      console.log(`build-web-host: [dry-run] patch ${manifestPath} with ${JSON.stringify(patch)}`)
      return
    }
    if (!existsSync(manifestPath)) {
      throw new Error(`build-web-host: ${manifestPath} missing — pnpm deploy did not produce a staged package.`)
    }
    if (!existsSync(join(this.staging, ENTRY_BIN))) {
      throw new Error(`build-web-host: ${join(this.staging, ENTRY_BIN)} missing — run without --skip-build so lib/ artifacts exist.`)
    }
    const manifest = JSON.parse(await readFile(manifestPath, 'utf8')) as Record<string, unknown>
    await writeFile(manifestPath, `${JSON.stringify({ ...manifest, ...patch }, null, 2)}\n`)
    console.log(`build-web-host: injected pkg config into ${manifestPath}`)
  }
  ...
  async pack(target: Target): Promise<string[]> {
    const productName = target.platform === 'macos' && target.arch === 'arm64'
      ? 'dsh-web-host'
      : `dsh-web-host-${target.platform}-${target.arch}`
    const product = join(this.outDir, productName)
    ...
    if (!this.cli.dryRun && !existsSync(product)) {
      throw new Error(`build-web-host: product ${product} is missing after the pkg run; inspect ${this.outDir}.`)
    }
    ...
    if (this.cli.dryRun) {
      console.log(`build-web-host: [dry-run] cp ${source} ${spawnHelper}`)
    } else {
  ...
  private async prepareNativePty(target: Target): Promise<void> {
    ...
    if (this.cli.dryRun) console.log(`build-web-host: [dry-run] rm -rf ${stagedBuild}`)
    ...
    if (this.cli.dryRun) {
      console.log(`build-web-host: [dry-run] cp ${source} ${destination}`)
      return
    }
    const host = Target.host()
    if (target.platform !== host.platform || target.arch !== host.arch) {
      throw new Error(
        'build-web-host: build the Linux runtime on its target architecture; '
  ...
  printProducts(products: string[]): void {
    console.log(this.cli.dryRun ? 'build-web-host: [dry-run] would produce:' : 'build-web-host: products:')
  ...
  private async run(label: string, command: string, args: string[]): Promise<void> {
    ...
    if (this.cli.dryRun) {
      console.log(`build-web-host: [dry-run] ${printable}`)
      return
    }
    console.log(`build-web-host: ${label}: ${printable}`)
    await new Promise<void>((resolvePromise, reject) => {
      ...
      child.once('exit', (code, signal) => {
        ...
        reject(new Error(`build-web-host: ${label} failed (${cause}): ${printable}`))
      })
    })
  }
}
...
async function main(): Promise<void> {
  ...
  const pipeline = new WebHostBuild(cli)
  console.log(`build-web-host: targets: ${cli.targets.map(target => target.spec).join(', ')}`)
  console.log(`build-web-host: staging: ${pipeline.staging}`)
