...
该壳不重实现 Client 包，也不新增 IPC `doFetch` 载体。它使用 [GUI 分层说明](../../implemented/architecture/2026-07-19-gui-layering-and-rpc-protocol.md) 已经交付的 HTTP 承载。原生菜单、`NSOpenPanel` 以及打包进应用的 web-host 可执行文件属于后续阶段，不进入本次改动。
...
1. 当该路径存在时使用 `DSH_BIN`。
2. 后续阶段把 `dsh-web-host` 可执行文件复制进应用后，使用该捆绑副本。
...
**把 Xcode 工程放在单独仓库。** 第 1 期壳必须解析本 checkout 的 `dsh` 源码启动和 `apps/web/dist`。
...
Linux CI 无法编译或运行该应用；损坏的 `project.pbxproj` 只能在 Mac 上发现。
