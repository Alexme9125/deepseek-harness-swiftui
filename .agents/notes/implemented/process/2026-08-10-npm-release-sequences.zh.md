...
`pack` 无凭据。本个人 fork 只通过 `workflow_dispatch` 启动 pack 与 publish（[按需 GitHub Actions](2026-08-19-fork-manual-github-actions.md)）；一次派发的 pack 仍能证明发布集可以完整打出来。`publish` 挂在 `npm-publish` environment 后面等人工审批，且既不构建也不重建——它上传的就是 pack 产出的字节。pack 的 run 按 ref 分组，不同 ref 上的并发派发不会互相顶掉；全局分组落在 publish job 上，因为 dist-tag 是共享的 registry 状态。
...
一次派发的 pack 会为两条序列完整打包（无凭据），并把打包好的 dsh tarball 装进一次性 consumer，用普通 Node 驱动 `dsh --version`。这个探针刻意只有一条命令：它证明 `files` 选出了完整 payload、发布出去的范围可解析，不涉及任何交互行为。
