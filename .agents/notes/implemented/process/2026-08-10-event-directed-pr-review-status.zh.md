...
本个人 fork 不为这两个工作流订阅 GitHub issue 或 pull-request 事件（[按需 GitHub Actions](2026-08-19-fork-manual-github-actions.md)）。生命周期作业的 `if` 仍编码请求修改过滤条件。[Issue 管理测试](../../../../.github/issue-management/policy.test.mjs) 中的处理映射仍是命令约定。

## 验证

[Issue 管理测试](../../../../.github/issue-management/policy.test.mjs)锁定事件到命令的映射、请求修改命令后重复请求评审所触发的状态转换、请求修改后的状态回退、终态保护，以及保留人工覆盖状态。[工作流测试](../../../../scripts/ci-workflow.spec.ts)锁定本 fork 没有自动 GitHub 事件，并保留请求修改作业的条件。
