...
Rewriting the Client plugin tree in SwiftUI would duplicate [`packages/client/*`](../../../../packages/client/AGENTS.md) and the [Connection RPC](../../../../packages/client/connection/src/rpc.ts) wire protocol. ACP and the SDK JSON-RPC protocol omit workspaces, settings, session resume, and the streaming transcript, so they cannot be the product window.
...
The shell does not reimplement Client packages or add an IPC `doFetch` carrier. It uses the HTTP carriage the [archived GUI layering note](../../archived/architecture/2026-07-19-gui-layering-and-rpc-protocol.md) already ships. Native File-menu actions, `NSOpenPanel`, and Dock or window folder drops dispatch a same-origin `dsh-native-command` event into that page so `ctx.workspaces` and SettingsRoot run the existing Web flows.

The `.app` may embed `dsh-web-host`, a closed `@yao-pkg/pkg --sea` executable of the web profile. That product is not [`dsh-jsonrpc-agent-pkg`](2026-07-10-single-file-executable-sdk-runtime-distribution.md): the JSON-RPC exe boots an external `cordis.yml` over stdio and has no Host webserver or frontend dist. The web-host deploy root is [`apps/macos/web-host/package.json`](../../../../apps/macos/web-host/package.json). Its packaged entry is [`apps/cli/src/packaged-bin.ts`](../../../../apps/cli/src/packaged-bin.ts), which calls `runProfile` with `resolutionMode: 'runtime'` so the official profile-resolution generation supplies bare plugins from the VFS ([profile resolution](2026-09-09-profile-resolution-generations.md)), including a later `loader.create` of a bare package such as the directory-picker backend. A pkg SEA also sets `process.pkg`, which forces the same mode. The client-modules node half resolves each `dsh.client` `package.json` from that same closed tree when the profile directory cannot ([host package lookup](../bug-fix/2026-08-14-client-modules-host-package-resolve.md)). The snapshot lists sharp's libvips shared libraries as pkg `assets` (`*.dylib`, `*.so`). pkg extracts the `@img` folder from the VFS when `dlopen` loads the sharp `.node` addon; dyld then follows `@rpath` on that real path. Omitting those assets leaves the addon on disk without libvips and the host exits during plugin init. Preset discovery lists snapshot roots by `readdir` names and `stat`, because pkg `--sea` returns strings even with `withFileTypes` ([preset roster](../bug-fix/2026-08-14-pkg-sea-readdir-returns-names.md)). App Sandbox stays off.
...
The shell binds a free `127.0.0.1` port, closes the probe socket, and passes `--host 127.0.0.1 --port <n>`. Readiness is a successful `GET /` at `http://127.0.0.1:<n>/`. The wait `connect`s TCP until the port accepts, then issues that GET, so CFNetwork does not log connection-refused for the closed-socket window. The WebView must open that IPv4 loopback origin, not `localhost`, so the existing `/api` trust fence still treats the page as loopback. Quit sends SIGTERM to the child and SIGKILL if it does not exit.
...
The Xcode scheme packages `dsh-web-host` on first Run when `apps/macos/dist/dsh-web-host` is missing. Xcode sets SRCROOT to `apps/macos`; scheme pre-actions and the launch working directory resolve the checkout as the ancestor of SRCROOT that contains `apps/cli/src/bin.ts`. [`scripts/copy-web-host.sh`](../../../../apps/macos/scripts/copy-web-host.sh) copies the host into `Contents/MacOS` and ad-hoc-signs it. On a full link the script runs before Xcode CodeSign, while the main executable and `DeepSeekHarness.debug.dylib` are still unsigned, so resealing then fails with `code object is not signed at all`. Incremental builds skip CodeSign; the script probes the main executable *before* replacing nested helpers and reseals the `.app` when that probe succeeds. After the copy, `codesign --verify` on the same Mach-O follows the broken bundle seal. A replaced nested helper that is not sealed dies with SIGKILL (status 9, `Code Signature Invalid`). The helper is not signed with Hardened Runtime: pkg extracts unsigned `.node` files to `~/.cache/pkg`. Linux CI cannot emit `node24-macos-arm64`. Intel Macs are out of scope.
