...
[`.github/workflows/macos-dmg-release.yml`](../../../../.github/workflows/macos-dmg-release.yml) 跑在 `macos-15`（Apple Silicon）上。推送 `macos-v*` tag 会发布该 tag；`workflow_dispatch` 会发布 `macos-snapshot-<sha>` 预发布。该作业安装依赖、运行 `pnpm run package:macos-app`，并以 `contents: write` 挂上那一个 `DeepSeekHarness-*-arm64.dmg`。Linux CI 仍然无法产出该镜像；该工作流就是打包机。
...
**买到：** 在本仓库上打 `macos-v*` tag 会产出一个 GitHub Release，其资产是 Apple Silicon DMG。README 写明这是 Alex 的个人项目、点出 Darwin 动漫社所属但不宣称社团所有权、披露 Cursor/Grok/Composer 作者身份，并告诉接收方：插件无法便捷装进这份 DMG，以及如何从源码添加或通过重建添加。
